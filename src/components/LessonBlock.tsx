import type { Lesson, ParamRow, Integration, ErrorItem } from "@/data/modules";
...
// Map lesson IDs → hero image key
const MAIN_MAP: Record<string, string> = {
  // Módulo 1 — Interface Completa
  "prepare-workspace": "main-interface",
  "layout-barra-superior": "lesson-layout-barra-superior",
  ...
  "analise-requisitos": "lesson-analise-requisitos", // Mod 8 — 8.1
  "pecas-funcionais": "lesson-pecas-funcionais",     // Mod 8 — 8.2
  "pecas-esteticas": "lesson-pecas-esteticas",       // Mod 8 — 8.3
  "mercado-padronizacao": "lesson-mercado-padronizacao", // Mod 9 — 9.1
  ...
  "diagnostico-metodologia": "lesson-diagnostico-metodologia", // Mod 14 — 14.1
  "stringing-ghosting": "lesson-stringing-ghosting",           // Mod 14 — 14.2
  "warping-delaminacao": "lesson-warping-delaminacao",         // Mod 14 — 14.3
    ...
    <section id={l.id} className={`scroll-mt-24 ${!isFirst && withDivider ? "mt-20 pt-16 border-t border-border" : ""}`}>
      ...
      {TOPIC_MAP[l.id] && (
        <figure className="mt-8 w-full rounded-2xl border border-primary/30 bg-card/40 overflow-hidden">
          <img src={TOPIC_MAP[l.id]} alt={l.title} loading="lazy" className="w-full h-auto aspect-video object-cover block" />
        </figure>
      )}
            ...
            {l.paramDetails.map((p, i) => (
              <div key={i} className="rounded-2xl border border-border bg-card/40 p-6">
                <div className="flex items-start justify-between gap-4 flex-wrap mb-4 pb-3 border-b border-border">
                  ...
                  <div className="mono text-sm text-primary bg-primary/10 px-3 py-1.5 rounded-md border border-primary/30">{p.value}</div>
                </div>
                <div className="grid gap-5">
                  ...
                  {p.optionsTable && <MiniTable label="▸ Tabela de opções" color="primary" data={p.optionsTable} />}
                  <div>
                  ...
                  {p.summaryTable && <MiniTable label={`▸ ${p.summaryTable.title ?? "Tabela resumo"}`} color="primary" data={p.summaryTable} />}
                </div>
              </div>
            ))}
          </div>
